...
<div class="uk-grid-large uk-child-width-1-2 uk-background-default uk-padding-small ee-border-bottom" uk-grid>
  <div>
    <h3 class="uk-text-primary">تعديل بيانات المستخدم</h3>
  </div>
  <div>
    <ul class="uk-breadcrumb uk-float-left">
      <li class="uk-text-small">
        <a href="{{ route('users.show', $user->id) }}" class="ee-en">
          {{ $user->name }}
        </a>
      </li>
...
<div class="uk-container uk-padding" id="createUser">
  <div class="uk-background-default ee-border">
    ...
    <form action="{{ route('users.update', $user->id) }}" class="uk-padding" method="POST">
      {{method_field('PUT')}}
      @csrf
      <fieldset class="uk-fieldset">
        ...
        <div class="uk-margin uk-grid-small uk-child-width-auto uk-grid">
          <label class="uk-text-small">
            <input class="uk-radio" type="radio" name="passwordOptions" value="oldPass" v-model="passwordOptions">
            لا تغير كلمة المرور
          </label>
          <label class="uk-text-small">
            <input class="uk-radio" type="radio" name="passwordOptions" value="newPass" v-model="passwordOptions">
            كلمة مرور جديدة
          </label>
        </div>{{-- Password Options --}}

        <div class="uk-margin uk-width-1-2" v-if="passwordOptions === 'newPass'">
